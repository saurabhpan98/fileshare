/**
 * WebRTC Peer-to-Peer File Transfer Manager
 * Direct device-to-device over WiFi — no server, no database.
 */
...
export class PeerManager {
  ...
  _handleMsg(event) {
    if (typeof event.data === 'string') {
      ...
      if (msg.type === 'file-start') {
        ...
        this.receiveExpected.delete(msg.fileId);
        this.currentReceiveId = null;
      ...
      if (this.currentReceiveId && this.receiveBuffers.has(this.currentReceiveId)) {
        this.receiveBuffers.get(this.currentReceiveId).push(event.data);
        this._updateProgress();
      }
    } else if (event.data instanceof Blob) {
      event.data.arrayBuffer().then((ab) => {
        if (this.currentReceiveId && this.receiveBuffers.has(this.currentReceiveId)) {
          this.receiveBuffers.get(this.currentReceiveId).push(ab);
          this._updateProgress();
        }
      });
    }
  ...
  _sendFile(file, fileId) {
    const header = JSON.stringify({ type: 'file-start', fileId, fileName: file.name, fileSize: file.size, mimeType: file.type });
    this.dc.send(header);
